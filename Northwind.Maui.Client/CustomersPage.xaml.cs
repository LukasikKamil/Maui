...
public partial class CustomersPage : ContentPage
{
	public CustomersPage()
	{
        ...
        try
        {
            ...
            InfoLabel.Text = $"BaseAdrress: {client.BaseAddress}";
            ...
            IEnumerable<CustomerDetailViewModel> customersFromService = response.Content.ReadFromJsonAsync<IEnumerable<CustomerDetailViewModel>>().Result;

            foreach (CustomerDetailViewModel c in customersFromService.OrderBy(customer => customer.CompanyName))
            {
                viewModel.Add(c);
            }
    ...
    async void Customer_Phoned(object sender, EventArgs e)
    {
        ...
        if (await DisplayAlert("Dial a Number", "Would you like to call " + c.Phone + "?", "Yes", "No"))
        {
            ...
            {
                await DisplayAlert(title: "Failed", message: string.Format("Failed to dial {0} due to: {1}",
                    c.Phone, ex.Message), cancel: "OK");
            }
        }
    }
